...
void SugraParameters::initialize() const {
  ...
  int err =
      sphenoSUGRA(p_tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, p_sgn, gMHu, gMHd,
                  gMl2, gMl3, gMr2, gMr3, gMq2, gMq3, gMu2, gMu3, gMd2, gMd3);
  error_parser("sphenoSUGRA", err);
}

void EwsbParameters::initialize() const {
  int err = sphenoEwsbMSSM();
  error_parser("sphenoEwsbMSSM", err);
}
...
Same as `spheno(params, settings)` but with a vector of parameters.
)pbdoc";

PYBIND11_MODULE(spheno, m) {
