...
void SugraParameters::initialize() const {
  ...
  int err =
      suspectSUGRA(p_tb, gMG1, gMG2, gMG3, gAl, gAt, gAb, p_sgn, gMHu, gMHd,
                   gMl2, gMl3, gMr2, gMr3, gMq2, gMq3, gMu2, gMu3, gMd2, gMd3);
  error_parser("suspectSUGRA", err);
}

void EwsbParameters::initialize() const {
  int err = suspectEwsbMSSM();
  error_parser("suspectEwsbMSSM", err);
}
...
Same as `suspect(params, settings)` but with a vector of parameters.
)pbdoc";

PYBIND11_MODULE(suspect, m) {
  py::module_::import("pymicromegas");
  m.doc() = "Python interface to micromegas with SoftSusy as an RGE backend.";
  // ================================================
  // ---- Single Parameter, SUGRA -------------------
  // ================================================
  m.def(
      "suspect",
      [](const SugraParameters &params,
